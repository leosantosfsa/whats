...
module.exports = {
  async up(queryInterface, Sequelize) {
    ...
    const insertedAnnouncements = await queryInterface.sequelize.query(
      `SELECT id FROM "Announcements" WHERE title = 'Principais Funcionalidades.';`
    );

    const announcementId = insertedAnnouncements[0][0].id;
    ...
    await queryInterface.bulkInsert('AnnouncementVisibilities', visibilityEntries, {});
  },
